 ...
 *   Date            By               Description
 * 
 * 16-Oct-93     Thang Trinh        Initial release (v1.0) for SunOS 4.1.x
 *                                  and Solaris 2.x.
 * 14-May-99     Thang Trinh        Version 3.0 release for all supported
 *                                  platforms.
 * 14 Sep 99     Tom Kuiper         Version 3.1 with fix in net_getpeername
 *                                  under Linux
 *
 * Description:
...
io_mode mode;                              /* listen socket I/O mode */
{
    ...
    static struct linger off = {1, 0};     /* linger flag for setsockopt() */
...
int namelen;                            /* hostname length in bytes */
{
    ...
    int peer_len = sizeof (peer);       /* length of peer's address */
